...
- **Инлайн статус/приоритет на карточке спринт-борда** (`SprintTaskCard`): портал-дропдауны прямо на карточке, без открытия drawer. Триггеры гасят `pointerdown`+`click` (иначе стартует drag / откроется drawer), закрываются по скроллу. Хендлеры (`handleInlineWorkflow`/`handleInlinePriority`) — оптимистичный `setTaskMap` + `updateTask`. Колонку задача НЕ меняет (колонка ≠ workflow_status в этой модели).
