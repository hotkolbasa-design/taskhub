...
- Компонент `AnalyticsView` получает `projects: ProjectAnalytics[]`. Селектор проекта (`ProjectSelect`, кастомный портал-дропдаун) — «Все проекты» (если проектов >1) или конкретный; при «Все» users/sprintsByUser мёржатся. Выбор проекта персистится (`usePersistedFilter`, ключ с `currentUserId`). Наверху — период текущей недели «Текущая неделя: dd.mm – dd.mm» (`fmtWeekRange`, `lib/utils/week.ts`). Два режима:
  - **Сводка** (`EfficiencySummary`, дефолт) — матрица: строки-сотрудники × колонки-недели. Ось недель выровнена по календарным неделям (понедельник ISO, `mondayOf` из `lib/utils/week.ts`), непрерывная от самой ранней недели с данными до текущей. Липкие крайние колонки: слева — сотрудник (аватар + имя + мини-спарклайн тренда + среднее %), справа — «Текущая» (подсвечена, показывает период недели). Прошлые недели скроллятся между ними (единый `overflow-auto`, `maxHeight:56vh`, автоскролл вправо). Ячейка = `EffPill` с `%` и цвет-порогом (`efficiencyColor`), пустая = «·». Значение считается заново через `computeEfficiency` по объединённым задачам недели.
  - **Индикатор спринта на этой неделе**: точка на аватаре — зелёная = начал НОВЫЙ спринт на текущей неделе (`mondayOf(date_from) === currentMonday`), жёлтая = активный спринт тянется с прошлой недели, серая = не начат. В колонке «Текущая»: `EffPill` (начал) / «идёт с dd.mm» / «не начат». Чип-сводка «Начали спринт: X/Y» + «Средняя: N%». Фильтр сотрудников (`UserFilter`) + выбор недель `8/12/Всё`, оба персистятся.
