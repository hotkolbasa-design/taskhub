...
- Фильтр по роли (исполнитель / постановщик, мультивыбор). **Дефолт зависит от роли:** админ/руководитель → оба (он в основном постановщик, иначе не видит задачи спринта, которые раздал команде); сотрудник → только исполнитель. См. `roleFilter` в `my-tasks-board.tsx`.
