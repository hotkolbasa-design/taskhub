...
- Страница `/analytics`, компонент `AnalyticsView` — два режима (переключатель вверху):
  - **Сводка** (`EfficiencySummary`, дефолт) — матрица: строки-сотрудники × колонки-недели. Ось недель выровнена по календарным неделям (понедельник ISO как ключ, `mondayOf`), непрерывная от самой ранней недели с данными до текущей. Липкие крайние колонки: слева — сотрудник (аватар + имя + мини-спарклайн тренда + среднее %), справа — «Текущая» (подсвечена). Прошлые недели скроллятся между ними (единый `overflow-auto`, автоскролл вправо к свежим). Ячейка = `EffPill` с `%` и цвет-порогом (`efficiencyColor`), пустая неделя = «·». Значение ячейки считается заново через `computeEfficiency` по объединённым задачам всех спринтов сотрудника в эту неделю (на случай нескольких проектов).
  - **Индикатор спринта на этой неделе**: точка на аватаре — зелёная = начал НОВЫЙ спринт на текущей неделе (`mondayOf(date_from) === currentMonday`), жёлтая = активный спринт тянется с прошлой недели, серая = не начат. В колонке «Текущая»: `EffPill` (начал) / «идёт с dd.mm» (тянется) / «не начат». Чип-сводка вверху «Начали спринт: X/Y» + «Средняя: N%».
  - Фильтр сотрудников — `UserFilter` (мультиселект, пусто = все); выбор недель `8 / 12 / Всё`. Оба состояния персистятся в localStorage приватно (`usePersistedFilter`, ключ с `currentUserId`). Клик по строке → открывает режим «По сотруднику» для этого человека.
  - **По сотруднику** — прежний детальный вид: слева список, справа активный спринт + история недель одного выбранного.
- Admin видит всех, сотрудник — только себя (или тех к кому есть доступ через `access_settings`). Данные грузятся на сервере: `getVisibleUsers` + `getUserSprintHistory` по каждому → `sprintsByUser`.
