...
- **Бэклог / Спринт**: фильтрует карточки по `assignee_id` (для всех, они и так видят все задачи проекта). **Важно:** при активном фильтре DnD ОТКЛЮЧАЕТСЯ (`useSensors(...(filterActive ? [] : [sensor]))`) — иначе переупорядочивание отфильтрованного подмножества сломает `backlog_order`/`column_order`. На бэклоге фильтр применяется только к списку бэклога (не к спринт-панели — у неё `onTasksChange` заменяет весь массив).
