...
- `localhost:3000` занят другим проектом пользователя — TaskHub там не тестируется
- После любых изменений кода: `git commit` + `git push` → Vercel автоматически деплоит
- Никогда не просить пользователя проверять localhost
